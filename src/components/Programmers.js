...
export default function Programmers() {
  ...
  const [awsome, setAwsome] = useState(listOfAwesome);
  const [currentAwsome, setCurrentAwsome] = useState(null);
  ...
  const getNameOfFeatured = () => {
    ...
    for (let i in awsome) {
      if (awsome[i].id === currentAwsome) {
        return awsome[i].name;
      }
    }
  };
  ...
  const style = {
    fontSize: "1.5em",
    marginTop: "0.5em",
    color: currentAwsome ? "gold" : "royalblue", // 🤔 color turns to gold, when celebrating
  };
